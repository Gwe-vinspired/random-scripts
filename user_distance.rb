...
# DEV.vinspired.com / distance linked to role

@avg = {}
@avg_bp = {}
def user_distance
  CSV.open("user_applications_distances.csv", "w",
    :write_headers=> true,
    :headers => ["User Id", "User Postcode", "Role Postcode", "Disctance (in miles)", "Has logged hours", "User Address", "Role Address", "Recorded Against"]
  ) do |csv|
    User.all.each_with_index do |u, index|
      if u.postcode
        short_pc = u.postcode.split(" ")[0] || u.postcode[0..2] || u.postcode[0..3]
        if short_pc
          if (LONDON_POSTCODES.include? short_pc.upcase) && (u.hours_recorded > 0)
            # p "true -- #{u.user_events.count}"
            applications = u.user_events
            applications = u.user_events.select { |ue| 
              x = u.time_records.to_a.map {|tr| tr.user_event_id} 
              x.include?(ue.id) 
            }
            if u.postcode
              ...
              if applications.length > 0
                applications.each do |a|
                  ...
                  # if event.postcode && (u.time_records.to_a.select {|tr| tr.user_event_id == a.id }).length > 0
                  if event.postcode
                    ...
                    if user_location.first && location.first
                      ...
                      # byebug
                      csv << ["#{u.id}", "#{u.postcode}","#{event.postcode}", "#{distance}", "#{u.hours_recorded}", "#{user_location.first.address}", "#{location.first.address}", "#{a.time_records.to_a.map {|tr| Event.find(tr.event.id).postcode }}"]
                    end
                  end            
                end 
              end 
